...
#
# und trainieren Sie eine Support Vektor Maschine (C=1) mit RBF Kernel (gamma=0.1) 
...
best_acc = 0
best_C = None
for gamma in np.geomspace(0.003,0.04, 20):
    bar = tqdm(np.geomspace(10, 100.0, 20))
    for C in bar:
        ...
        bar.set_description(f"acc: {correct_test*100:.2f}% ({best_acc*100.0:.2f}%), C={C:8.3f} ({best_C:8.3f}), gamma={gamma:.3f} ({best_gamma:.3f})")
